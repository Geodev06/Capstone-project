...
<head>
    ...
    <title>{{ $data[0]['payment'][0]->payment_id}}</title>
            ...
            <div class="col-lg-6 mx-auto my-5">
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p>{{ $data[0]['payment'][0]->payment_id}}</p>
                    </div>
                    <div class="col-md-6">
                        <h6 class="fw-bold">PAYER EMAIL</h6>
                        <p>{{ $data[0]['payment'][0]->payer_email}}</p>
                    </div>
                    <div class="col-md-6">
                        <h6 class="fw-bold">DATE OF PAYMENT</h6>
                        <p>{{ $data[0]['payment'][0]->created_at->format('M d Y')}}</p>
                    </div>
                    <div class="col-md-6">
                        <h6 class="fw-bold">PAYMENT STATUS</h6>
                        <p>{{ $data[0]['payment'][0]->payment_status}}</p>
                    </div>
                </div>
                <hr>
                ...
                <div class="row mt-2">
                    ...
                    <div class="col-md-12">
                        ...
                        <p>{{ $data[0]['details'][0]->target_date}}</p>
                    </div>

                    <div class="col-md-12">
                        <h6 class="fw-bold">HEAD NO.</h6>
                        <p>{{ $data[0]['details'][0]->no_of_person}}</p>
                    </div>
...
<script>
    var value = "{{ $data[0]['payment'][0]->amount}}"
    ...
    function makeCode(data, filename) {
        ...
        setTimeout(() => {
            ...
            dl.setAttribute('download', "{{ $data[0]['payment'][0]->payment_id }}")
            dl.removeAttribute('hidden')
        }, 500)
    }
    const data = "{{ route('entrance.receipt',$data[0]['payment'][0]->payment_id)}}"
