...
<body>
    <div class="d-flex" id="wrapper">
        ...
        <div class="border-end bg-white shadow-lg" id="sidebar-wrapper">
            <div class="sidebar-heading border-bottom bg-light p-5"><a class="text-dark" href="{{ route('user.dashboard') }}" style="text-decoration: none;"><span class="fw-bold">TANAW APP</span></a></div>
            <div class="list-group list-group-flush">
                <a class="list-group-item list-group-item-action p-3 {{ 'user/dashboard' === request()->path()? 'active-link' :''}}" href="{{ route('user.dashboard') }}"> <i class="bx bxs-dashboard"></i> Dashboard</a>
                <a class="list-group-item list-group-item-action p-3 {{ 'user/nearby' === request()->path()? 'active-link' :''}}" href="{{ route('user.nearby') }}"><i class=" bx bx-map"></i> Nearby TANAW</a>
                <a class="list-group-item list-group-item-action p-3 {{ 'user/tanaw-map' === request()->path()? 'active-link' :''}}" href="{{ route('tanaw.map') }}"><i class=" bx bx-map-alt"></i> TANAW Map</a>


                <a class="list-group-item list-group-item-action p-3 {{ 'user/entrance' === request()->path()? 'active-link' :''}}" href="{{ route('user.entrance') }}"><i class=" bx bx-user-check"></i> Entrance Registration</a>
                <a class="list-group-item list-group-item-action p-3 {{ 'user/bookings' === request()->path()? 'active-link' :''}}" href="{{ route('user.bookings') }}"><i class=" bx bx-bookmarks"></i> My Bookings</a>
                <a class="list-group-item list-group-item-action p-3 {{ 'user/my-payments' === request()->path()? 'active-link' :''}}" href="{{ route('user.payments') }}"><i class=" bx bx-credit-card"></i> Recent payments</a>
                <a class="list-group-item list-group-item-action p-3 {{ 'user/messages' === request()->path()? 'active-link' :''}}" href="{{ route('user.messages') }}"><i class=" bx bx-chat"></i> Send Message @if($unread_count > 0) <span class="badge bg-danger">{{$unread_count}}</span> @endif</a>
            </div>
        </div>
        ...
        <div id="page-content-wrapper">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="navbar-nav ms-auto mt-2 mt-lg-0">
                            <li class="nav-item dropdown">
                                <a class="nav-link dropdown-toggle" id="navbarDropdown" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="avatar p-3 py-2 text-center m-2">{{ $userdata->name[0]}}</span>{{ $userdata->name}}</a>
                                <div class="dropdown-menu dropdown-menu-end p-2" aria-labelledby="navbarDropdown">
                                    <a class="dropdown-item" id="btn-notifications"><i class="bx bx-bell"></i> Notifications @if($notifications > 0) <span class="badge bg-danger">{{ $notifications}}</span> @endif</a>
                                    <a class="dropdown-item {{ 'user/setting' === request()->path()? 'active-link' :''}}" href="{{ route('user.setting') }}"> <i class="bx bx-cog"></i> Account setting</a>
            ...
            <script>
                ...
                function update_notification(id) {
                    var route_ = "{{ route('user.notification.update',':id') }}"
                    $.ajax({
                ...
                function delete_notification(id) {
                    var route_delete = "{{ route('user.notification.delete',':id') }}"
                    $.ajax({
            ...
            @if(!empty($eligible) && $eligible === true)

            <link rel="stylesheet" href="{{ asset('user/rating.css')}}" />
            ...
            <div class="container-fluid">
                ...
                @if('user/setting' === request()->path())
                @yield('setting')
                @endif


                @if('user/messages' === request()->path())
                @yield('messages')
                @endif

                @if('user/bookings' === request()->path())
                @yield('booking')
                @endif
                ...
                @if('user/my-payments' === request()->path())
                @yield('payments')
                @endif

                @if('user/entrance' === request()->path())
                @yield('entrance')
                @endif

                @if('user/tanaw-map' === request()->path())
                @yield('tanaw')
                @endif

                @if(count($rooms) > 0)

                @if('user/view-room/'.$rooms[0]->room_no === request()->path())
                @yield('room_info')
                @endif

                @if('user/check-in-form/'.$rooms[0]->room_no === request()->path())
                @yield('check_in_form')
                @endif
                ...
                @if('user/dashboard' === request()->path())
                ...
                <div class="row p-5" style="background-color: #f2f2f2;">
                    ...
                    <div class="col-lg-12 mb-3">
                        ...
                        <div class="card shadow-sm p-4">
                            <div class="d-flex flex-column">
                                <h1 class="text-dark">{{ now()->format(' jS \of F, Y.  g:i  A') }}</h5>
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-3 mb-3 ">
                        <div class="my-gradient h-100 my-border p-4 d-flex justify-content-between align-items-center">
                            <div class="d-flex flex-column">
                                <h5>REGISTRATIONS</h5>
                                <p style="font-size: 12px;color:springgreen;">Total no. of registrations made. <span class="text-white">{{$dashboard_data['registration']}}</span></p>
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-3 mb-3">
                        <div class="my-gradient h-100 my-border p-4 d-flex justify-content-between align-items-center">
                            <div class="d-flex flex-column">
                                <h5>MY BOOKINGS</h5>
                                <p style="font-size: 12px; color:springgreen;">Total no. of bookings made. <span class="text-white">{{$dashboard_data['bookings']}}</span></p>
                            </div>
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-3 mb-3">
                        <div class="my-gradient h-100 my-border p-4 d-flex justify-content-between align-items-center">
                            <div class="d-flex flex-column">
                                <h5>NEARBY</h5>
                                <p style="font-size: 12px;color:springgreen;">Other things to do in Rizal. <span class="text-white">{{$dashboard_data['nearby']}}</span></p>
                            </div>
                    ...
                    <div class="col-sm-6 col-md-6 col-lg-3 mb-3">
                        <div class="my-gradient h-100 my-border p-4 d-flex justify-content-between align-items-center">
                            <div class="d-flex flex-column">
                                <h5>NOTIFICATIONS</h5>
                                <p style="font-size: 12px;color:springgreen;">My notifications. <span class="text-white">{{$dashboard_data['notifications']}}</span></p>
                            </div>
                            ...
                            <div class="room-container row p-4">
                                ...
                                <div class="col-md-6 col-lg-4 mb-4 mt-2">
                                    <div class="room-card h-100 p-5 d-flex flex-column">
                                        <h5 class="text-white">Room no. {{ $room->room_no}}</h5>
                                        <span class="text-info" style="font-size: 12px;"><i class="bx bx-check text-success"></i> Available</span>
                                        <span class="text-warning" style="font-size: 12px;"><i class="bx bx-check text-success"></i> {{ $room->min .'-'. $room->max}} person.</span>
                                        <a class="btn-book-now mt-4" data-id="{{$room->room_no}}"> Book now</a>
                                    </div>
                                </div>
                                @endforeach
                            </div>
