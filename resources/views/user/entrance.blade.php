...
<div class="row p-5" style="height: 100vh; background-color: #f2f2f2;">
    ...
    <div class="col-md-12">
        ...
        <form action="{{ route('user.entrance.pay') }}" method="POST">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    ...
                    <div class="input-group">
                        <input id="date" type="date" value="{{ old('date') }}" name="date" autocomplete="off" class="form-control" />
                    </div>
                    <div class="input-group mt-2">
                        <input type="number" id="no_of_person" value="{{ old('no_of_person') }}" placeholder="No. of person" name="no_of_person" autocomplete="off" class="form-control" />
                    </div>
                </div>
                <div>
...
<script>
    $(function() {
        ...
        function load_data() {
            ...
            return $.ajax({
                ...
                for (let i = 0; i < data.entrance.length; i++) {
                    var button = '<button data-date="' + data.entrance[i].date + '" data-status="' + data.entrance[i].payment_status + '" data-user_id="{{$userdata->id}}" data-amount="' + data.entrance[i].amount + '" data-payment_id="' + data.entrance[i].payment_id + '" data-payer_email="' + data.entrance[i].payer_email + '" class="btn btn-sm btn-dark btn-more">More</button>'
        ...
        $('#table-entrance tbody').on('click', 'tr td .btn-more', function() {
            ...
            var route = "{{ route('user.entrance.getdetails',':payment_id') }}";
        ...
        $('#btn-receipt').on('click', function() {
            var link = "{{ route('entrance.receipt',':id')}}"
        ...
        $('#btn-refund').click(function() {
            ...
            if ($(this)[0].dataset.status == 'to-approve') {
                Swal.fire({
                    ...
                    if (result.value) {
                        var route = "{{ route('request.refund',':id') }}"
                        return $.ajax({
                ...
                if ($('#no_of_person').val() != '') {
                    ...
                    $.ajax({
                        ...
                        if (data.status == 200) {
                            ...
                            window.location.replace("{{ route('user.entrance')}}")
                        }
                    }).fail(function(e) {
